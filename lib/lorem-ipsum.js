...
export default class LoremIpsum {
   ...
   */
  getRandomWord(size) {
    return randNth(w.wordLists[size - 1]);
  }
   ...
   */
  getRandomFragment() {
    const pattern = randNth(w.fragmentPatterns);
  ...
  getRandomParagraph(size) {
    // const twice = size =>
    //   arrayOfSize(2).map(() => this.getRandomParagraph(size)).join("");
    const twice = size =>
      arrayOfSize(2, () => this.getRandomParagraph(size)).join("");
    ...
    if (size === w.SIZE_ANY) {
      ...
      return arrayOfSize(sentenceCount)
        .map(
          () =>
            this.getRandomSentence(w.SIZE_ANY).replace(/\b\w/, s =>
              s.toUpperCase()
            ) + ". "
        )
        .join("")

...
function updateConfig() {
  ...
  })[atom.config.get("lorem.defaults.defaultUnitType")];
  const DEFAULT_UNIT_SIZE = ({
    ...
    Long: 3,
    "Very Long": 4,
  })[atom.config.get("lorem.defaults.defaultUnitSize")];

  const DEFAULT_UNIT_COUNT = atom.config.get("lorem.defaults.defaultUnitCount");
  const DEFAULT_IS_WRAPPED = atom.config.get("lorem.defaults.defaultIsWrapped");
  const DEFAULT_WRAP_WIDTH = atom.config.get("lorem.defaults.defaultWrapWidth");
  const DEFAULT_IS_HTML = atom.config.get("lorem.defaults.defaultIsHTML");
  const DEFAULT_SHOW_HELP = atom.config.get("lorem.defaults.defaultShowHelp");
  ...
  let splitReg = atom.config.get("lorem.commands.splitRegExp");
  if (splitReg) {
    SPLIT_REG_EXP = new RegExp(splitReg.join("|"));
  }
}

...
export default class LoremIpsum {
   ...
   */
  getRandomFragment() {
    return randNth(w.fragmentPatterns)
  ...
  getRandomParagraph(size) {
    if (size === w.SIZE_ANY) {
      ...
      return sizedArray(sentenceCount)
        .map(() => sentenceSyntax(this.getRandomSentence(w.SIZE_ANY)))
        .join("")
        .trim();
    } else {
      return sizedArray(2)
  ...
  getRandomSentences(count, size) {
    return sizedArray(count)
      .map(() => sentenceSyntax(this.getRandomSentence(size)))
      .join("\n\n")
      .trim();
  }
  ...
  parseCommand(command) {
    ...
    for (let command of commandArray) {
      ...
      if (/^([a-z\?]+)(\d*)$/.test(command)) {
        [, str, num] = command.match(/^([a-z\?]+)(\d*)$/);
      } else if (/^(\d*)([a-z\?]+)$/.test(command)) {
        [, num, str] = command.match(/^(\d*)([a-z\?]+)$/);
      } else {
